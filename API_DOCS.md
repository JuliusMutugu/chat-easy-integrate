...
app.get("/projects/:id/chat", async (req, res) => {
  ...
  const response = await fetch(
    `https://chat-easy-integrate.onrender.com/api/rooms/${project.chatRoomId}`
  );
  const chatRoom = await response.json();
...
function ChatComponent({ roomId, username }) {
  const { messages, users, sendMessage, isConnected } = useChat(
    roomId,
    username
  );
...
_This API is designed to be simple yet powerful, allowing you to integrate real-time messaging into any application with minimal effort._
